...
data_filter$Voltage<-as.numeric(type.convert(as.character(data_filter$Voltage), dec = "."))
data_filter$Global_reactive_power<-as.numeric(type.convert(as.character(data_filter$Global_reactive_power), dec = "."))


par(mfrow = c(2, 2))
plot(data_filter$DT,data_filter$Global_active_power,type = "l",xlab=" ",ylab="Global Active Power") 

plot(data_filter$DT,data_filter$Voltage,type = "l",xlab="datetime",ylab="Voltage") 
...
legend("topright", inset=-0.09,lwd=1, col = c("black","red","blue"), legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),box.col=0)
plot(data_filter$DT,data_filter$Global_reactive_power,type = "l",xlab="datetime",ylab="Global_reactive_power") 
dev.copy(png, file = "F:/mala/coursera/Plot4.png") 
dev.off()
